...
ggplot(USArrests, aes(x = Murder, fill = UrbanPop_cat)) +
  ...
  labs(title = "Distribución de Asesinatos en Función de la Población Urbana/Rural",
       ...
       y = "Densidad") +
  scale_fill_manual(values = c("Más Población Urbana" = "steelblue", "Más Población Rural" = "tomato")) +
  theme_minimal()
```
...
```{r}
# Discretizar la variable UrbanPop
USArrests$UrbanPop_cat <- ifelse(USArrests$UrbanPop > 50, "Estados Urbanos", "Estados rurales")

# Crear el histograma para comparar la variable Murder
ggplot(USArrests, aes(x = Murder, fill = UrbanPop_cat)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 20, show.legend = TRUE) + 
  labs(title = "Histograma de Asesinatos según Población Urbana/Rural",
       x = "Número de Asesinatos",
       y = "Frecuencia") +
  scale_fill_manual(values = c("Estados Urbanos" = "lightblue", "Estados rurales" = "tomato")) +
  theme_minimal()

```
Con este gráfico vemos que, claramente hay más asesinatos en los estados urbanos, con una distribución más cercana a la normal, que en los estados rurales, donde además estos asesinatos se distribuyen de manera uniforme
